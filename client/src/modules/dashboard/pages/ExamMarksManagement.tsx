...
import ExamMarksFilterModal from '../components/ExamMarksFilterModal';
import ExamMarksViewModal from '../components/ExamMarksViewModal';
...
interface ExamMarkItem {
	...
	academic_name?: string | null; // Alternative field name from API
}
...
export default function ExamMarksManagement() {
	...
	const [examMarks, setExamMarks] = useState<ExamMarkItem[]>([]);
	...
	const [selectedExamMark, setSelectedExamMark] = useState<ExamMarkItem | null>(null);
	const [formDataToSubmit, setFormDataToSubmit] = useState<Partial<ExamMarkItem> | Partial<ExamMarkItem>[] | null>(null);
	const [formLoading, setFormLoading] = useState(false);
	const [deleteLoading, setDeleteLoading] = useState(false);
	const [publishLoading, setPublishLoading] = useState(false);
	
	// Selection state for bulk publish
	const [selectedExamMarkIds, setSelectedExamMarkIds] = useState<Set<string>>(new Set());
	...
	const fetchExamMarks = async () => {
		...
		try {
			...
			const { data } = await api.get(`/exam-marks/?school_id=${schoolId}&page=1&page_size=100&_t=${timestamp}`);
			
			// Handle paginated response
			console.log('Exam marks API response:', {
				rawResponse: data,
				hasItems: !!data?.items,
			...
			console.log('Processed exam marks:', {
				marksDataLength: marksData.length,
				allMarksLength: allMarks.length,
				firstMark: allMarks[0],
			});
			...
			console.log('Normalized exam marks:', {
				normalizedLength: normalizedData.length,
				firstNormalized: normalizedData[0],
			});
			
			// Force state update by creating a new array reference
			setExamMarks([...normalizedData]);
			
			console.log('Final exam marks state:', {
				total: normalizedData.length,
				sample: normalizedData[0],
			});
		} catch (error: any) {
			console.error('Error fetching exam marks:', error);
			const errorMessage = error.response?.data?.detail || error.message || 'Failed to load exam marks';
	...
	// Filter and paginate exam marks
	const filteredExamMarks = useMemo(() => {
		let filtered = examMarks.filter((record) => !record.is_deleted);
	...
	// Pagination
	const totalPages = Math.ceil(filteredExamMarks.length / itemsPerPage);
	const startIndex = (currentPage - 1) * itemsPerPage;
	const paginatedExamMarks = useMemo(() => {
		return filteredExamMarks.slice(startIndex, startIndex + itemsPerPage);
	}, [filteredExamMarks, startIndex, itemsPerPage]);

	// Analytics
	const analytics = useMemo(() => {
		const total = filteredExamMarks.length;
		const average = filteredExamMarks.length > 0
			? filteredExamMarks.reduce((sum, record) => sum + (record.exam_mark || 0), 0) / filteredExamMarks.length
			: 0;
		const highest = filteredExamMarks.length > 0
			? Math.max(...filteredExamMarks.map((record) => record.exam_mark || 0))
			: 0;
		const lowest = filteredExamMarks.length > 0
			? Math.min(...filteredExamMarks.map((record) => record.exam_mark || 0))
			: 0;
		const published = filteredExamMarks.filter((record) => record.is_published).length;
	...
	const handleEdit = (record: ExamMarkItem) => {
		setSelectedExamMark(record);
		setFormDataToSubmit(null);
		setUpdateConfirmOpen(false);
		setEditModalOpen(true);
	};

	const handleView = (record: ExamMarkItem) => {
		setSelectedExamMark(record);
		setViewModalOpen(true);
	};

	const handleDelete = (record: ExamMarkItem) => {
		setSelectedExamMark(record);
	...
	const handleSelectAll = (checked: boolean) => {
		if (checked) {
			const allIds = new Set(paginatedExamMarks.map((mark) => mark.exam_mark_id));
			setSelectedExamMarkIds(allIds);
		} else {
			setSelectedExamMarkIds(new Set());
		}
	};

	const handleSelectOne = (examMarkId: string, checked: boolean) => {
		const newSelected = new Set(selectedExamMarkIds);
		if (checked) {
			newSelected.add(examMarkId);
		} else {
			newSelected.delete(examMarkId);
		}
		setSelectedExamMarkIds(newSelected);
	};

	// Bulk publish handler
	const handleBulkPublish = async () => {
		if (selectedExamMarkIds.size === 0) {
			toast.error('Please select at least one exam mark to publish');
			return;
		}
		...
		try {
			...
			const response = await api.post(`/exam-marks/bulk-publish?school_id=${schoolId}`, {
				exam_mark_ids: Array.from(selectedExamMarkIds),
				is_published: true,
			});
			
			toast.success(response.data.message || `Successfully published ${selectedExamMarkIds.size} exam mark(s)`);
			setSelectedExamMarkIds(new Set());
			fetchExamMarks();
		} catch (error: any) {
			const errorMessage = error.response?.data?.detail || error.message || 'Failed to publish exam marks';
			toast.error(errorMessage);
		} finally {
	...
	// Unpublish single exam mark handler
	const handleUnpublish = async (record: ExamMarkItem) => {
		if (!record.exam_mark_id || !schoolId) {
		...
		try {
			setPublishLoading(true);
			const response = await api.post(`/exam-marks/bulk-publish?school_id=${schoolId}`, {
				exam_mark_ids: [record.exam_mark_id],
				is_published: false,
			});
			
			toast.success(response.data.message || 'Exam mark unpublished successfully');
			fetchExamMarks();
		} catch (error: any) {
			const errorMessage = error.response?.data?.detail || error.message || 'Failed to unpublish exam mark';
			toast.error(errorMessage);
		} finally {
			setPublishLoading(false);
		}
	...
	const handleCreateSubmit = async (data: Partial<ExamMarkItem> | Partial<ExamMarkItem>[]) => {
		setFormDataToSubmit(data);
		setCreateModalOpen(false);
		setCreateConfirmOpen(true);
	};

	const handleUpdateSubmit = async (data: Partial<ExamMarkItem>) => {
		setFormDataToSubmit(data);
	...
	const handleCreateConfirm = async () => {
		...
		try {
			...
			if (isBulk) {
				// Bulk submission - create multiple exam mark records
				const promises = (formDataToSubmit as Partial<ExamMarkItem>[]).map((item) => {
					const payload: any = {
						school_id: schoolId,
						std_id: item.std_id || '',
						...
						exam_mark: item.exam_mark || 0,
						exam_avg_mark: item.exam_avg_mark || null,
						status: item.status || null,
						is_published: item.is_published || false,
					};
					return api.post('/exam-marks/', payload);
				});

				await Promise.all(promises);
				toast.success(`${formDataToSubmit.length} exam mark(s) created successfully!`);
			} else {
				// Single submission
				const payload: any = {
					school_id: schoolId,
					std_id: (formDataToSubmit as Partial<ExamMarkItem>).std_id || '',
					subj_id: (formDataToSubmit as Partial<ExamMarkItem>).subj_id || '',
					cls_id: (formDataToSubmit as Partial<ExamMarkItem>).cls_id || '',
					academic_id: (formDataToSubmit as Partial<ExamMarkItem>).academic_id || '',
					term: (formDataToSubmit as Partial<ExamMarkItem>).term || '',
					exam_mark: (formDataToSubmit as Partial<ExamMarkItem>).exam_mark || 0,
					exam_avg_mark: (formDataToSubmit as Partial<ExamMarkItem>).exam_avg_mark || null,
					status: (formDataToSubmit as Partial<ExamMarkItem>).status || null,
					is_published: (formDataToSubmit as Partial<ExamMarkItem>).is_published || false,
				};

				await api.post('/exam-marks/', payload);
			...
			const errorMessage = error.response?.data?.detail || error.message || 'Failed to create exam mark';
			toast.error(errorMessage);
	...
	const handleUpdateConfirm = async () => {
		if (!formDataToSubmit || !selectedExamMark || !schoolId) {
			toast.error('Missing required data. Please try again.');
			return;
		}
		...
		try {
			setFormLoading(true);
			const payload: any = {
				...(formDataToSubmit as Partial<ExamMarkItem>),
			};

			await api.put(`/exam-marks/${selectedExamMark.exam_mark_id}?school_id=${schoolId}`, payload);
			...
			const errorMessage = error.response?.data?.detail || 'Failed to update exam mark';
			toast.error(errorMessage);
		} finally {
			setFormLoading(false);
		}
	...
	const handleDeleteConfirm = async () => {
		if (!selectedExamMark || !schoolId) return;

		try {
			setDeleteLoading(true);
			await api.delete(`/exam-marks/${selectedExamMark.exam_mark_id}?school_id=${schoolId}`);
			...
			toast.error(error.response?.data?.detail || 'Failed to delete exam mark');
		} finally {
				...
				<div className="flex-1 flex flex-col min-h-screen overflow-hidden lg:ml-64">
					...
					<main className="flex-1 overflow-y-auto p-6 flex items-center justify-center">
						<div className="text-center">
							...
							<p className="mt-4 text-gray-600">Loading exam marks...</p>
						</div>
					</main>
				</div>
			</div>
	...
	return (
		<div className="flex bg-gray-50 min-h-screen">
			...
			<div className="flex-1 flex flex-col min-h-screen overflow-hidden lg:ml-64">
				...
				<main className="flex-1 overflow-y-auto p-6 space-y-6">
					...
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
						<div>
							<h1 className="text-3xl font-bold text-gray-900">Exam Marks Management</h1>
							<p className="text-gray-600 mt-1">Track and manage student exam marks</p>
						</div>
						<button
					...
					<div className="bg-white rounded-[3px] shadow-sm border border-gray-200 p-6">
						<div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
							...
							<div className="flex items-center gap-3 flex-wrap">
								...
								{selectedExamMarkIds.size > 0 && (
									<div className="flex items-center gap-2 px-3 py-1 bg-primary-100 text-primary-700 rounded-[3px] text-sm font-medium">
										{selectedExamMarkIds.size} selected
									</div>
								)}
								<button
									onClick={handleBulkPublish}
									disabled={selectedExamMarkIds.size === 0 || publishLoading}
									className={`px-4 py-2.5 rounded-[3px] font-medium transition-all duration-200 flex items-center gap-2 ${
										selectedExamMarkIds.size === 0 || publishLoading
						...
						<div className="overflow-x-auto">
							<table className="min-w-full divide-y divide-gray-200">
								<thead className="bg-gray-50">
									<tr>
										<th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-12">
											<input
												type="checkbox"
												checked={paginatedExamMarks.length > 0 && paginatedExamMarks.every((mark) => selectedExamMarkIds.has(mark.exam_mark_id))}
								...
								<tbody className="bg-white divide-y divide-gray-200">
									...
									) : paginatedExamMarks.length === 0 ? (
										<tr>
											<td colSpan={11} className="px-6 py-8 text-center">
												<div className="flex flex-col items-center justify-center gap-3">
													<AcademicCapIcon className="w-12 h-12 text-gray-400" />
													<p className="text-gray-500 font-medium">No exam marks found</p>
												</div>
											</td>
										</tr>
									) : (
										paginatedExamMarks.map((record) => (
											<tr key={record.exam_mark_id} className="hover:bg-gray-50">
												<td className="px-6 py-4 whitespace-nowrap">
													<input
														type="checkbox"
														checked={selectedExamMarkIds.has(record.exam_mark_id)}
														onChange={(e) => handleSelectOne(record.exam_mark_id, e.target.checked)}
														className="rounded border-gray-300 text-primary-600 focus:ring-primary-500"
													/>
												</td>
												<td className="px-6 py-4 whitespace-nowrap">
												...
												<td className="px-6 py-4 whitespace-nowrap">
													<div className="text-sm text-gray-900">{record.academic_year_name || 'N/A'}</div>
												</td>
												<td className="px-6 py-4 whitespace-nowrap">
												...
												<td className="px-6 py-4 whitespace-nowrap">
													<div className="text-sm font-medium text-gray-900">
														{record.exam_mark !== null && record.exam_mark !== undefined 
															? Number(record.exam_mark).toFixed(1) 
															: '0.0'}
													</div>
												</td>
												<td className="px-6 py-4 whitespace-nowrap">
													<div className="text-sm text-gray-900">
														{record.exam_avg_mark !== null && record.exam_avg_mark !== undefined 
															? Number(record.exam_avg_mark).toFixed(1) 
															: 'N/A'}
													</div>
												</td>
												<td className="px-6 py-4 whitespace-nowrap">
							...
							<div className="bg-gray-50 px-6 py-4 flex items-center justify-between border-t border-gray-200">
								<div className="flex items-center gap-4">
									<span className="text-sm text-gray-700">
										Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredExamMarks.length)} of{' '}
										{filteredExamMarks.length} records
									</span>
									<select
										value={itemsPerPage}
										onChange={(e) => {
			...
			<Modal isOpen={createModalOpen} onClose={() => {
				...
				setSelectedAcademicYearId('');
				setFilteredStudents([]);
			}} title="Enter Exam Marks" size="2xl">
				<ExamMarksForm
					testMark={null}
			...
			<ExamMarksViewModal
				...
				onEdit={() => {
					...
					handleEdit(selectedExamMark!);
				}}
				onDelete={() => {
					setViewModalOpen(false);
					handleDelete(selectedExamMark!);
				}}
			/>
			...
			<ConfirmModal
				...
				title="Create Exam Marks"
				message={
					Array.isArray(formDataToSubmit)
						? `Are you sure you want to create ${formDataToSubmit.length} exam mark(s)?`
						: `Are you sure you want to create this exam mark?`
				}
				confirmText="Create"
				cancelText="Cancel"
				type="info"
			...
			<ConfirmModal
				...
				message={`Are you sure you want to update this exam mark?`}
				confirmText="Update"
				cancelText="Cancel"
				type="warning"
				loading={formLoading}
			...
			<ConfirmModal
				...
				message={`Are you sure you want to delete this exam mark? This action cannot be undone.`}
				confirmText="Delete"
				cancelText="Cancel"
				type="danger"
