"""
Migration script to add unique constraint on exam_marks table
...
async def add_unique_constraint():
    """Add unique constraint to exam_marks table"""
    async with engine.begin() as conn:
        # Check if constraint already exists
        check_constraint_query = text("""
            SELECT constraint_name 
            FROM information_schema.table_constraints 
            WHERE table_name = 'exam_marks' 
            AND constraint_type = 'UNIQUE'
            AND constraint_name = 'uq_exam_marks_student_subject_class_academic_term'
        """)
        ...
        find_duplicates_query = text("""
            SELECT 
                ...
                COUNT(*) as count,
                array_agg(exam_mark_id ORDER BY created_at DESC) as mark_ids
            FROM exam_marks
        ...
        create_index_query = text("""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_exam_marks_student_subject_class_academic_term
            ON exam_marks (school_id, std_id, subj_id, cls_id, academic_id, term)
            WHERE is_deleted = false
        """)
        ...
        verify_query = text("""
            ...
            AND indexname = 'uq_exam_marks_student_subject_class_academic_term'
        """)
...
if __name__ == "__main__":
    print("=" * 60)
    print("Adding Unique Constraint to exam_marks table")
    print("=" * 60)
